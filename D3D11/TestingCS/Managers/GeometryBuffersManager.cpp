...
namespace 
{
    void buildBuffers(ID3D11Device& device, GeometryBuffers& geometryBuffers)
    {
        ...
        //
        MeshData grid;
        GeometryGenerator::createFullscreenQuad(grid);
        ...
        for (size_t vertexIndex = 0; vertexIndex < totalVertexCount; ++vertexIndex)
        {
            vertices.push_back(
                Vertex(
                    grid.mVertices[vertexIndex].mPosition, 
                    grid.mVertices[vertexIndex].mNormal,
...
{   
    ...
    {
        buildBuffers(device, geometryBuffers);
    }
